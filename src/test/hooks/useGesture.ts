...
export const useGesture = (
        ...
        (currentValue, previousValue) => {
            if (currentValue !== previousValue) {
                if (draggedItemIdDerived.value !== null && item.id === draggedItemIdDerived.value) {
                    top.value = withSpring(currentSongPositionsDerived.value[item.id].updatedIndex * SONG_HEIGHT);
                } else {
                    top.value = withTiming(currentSongPositionsDerived.value[item.id].updatedIndex * SONG_HEIGHT, { duration: 500 });
                }
            }
        }
    );
    ...
    const gesture = Gesture.Pan()
        .onStart(() => {
            ...
            currentIndex.value = currentSongPositionsDerived.value[item.id].updatedIndex;
        })
        .onUpdate((e) => {
            ...
            const newTop = currentSongPositionsDerived.value[draggedItemIdDerived.value].updatedTop + e.translationY;
            ...
            if (newIndex.value !== currentIndex.value) {
                ...
                const currentDragIndexItemKey = getKeyOfValue(currentIndex.value, currentSongPositionsDerived.value);
